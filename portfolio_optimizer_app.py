...
E_rB = st.sidebar.number_input(
    "Expected Return E(r_B) (%)",
    value=5.0,
    min_value=0.0,
    max_value=50.0,
    step=0.1,
) / 100
sigma_B = st.sidebar.number_input(
...
E_rS = st.sidebar.number_input(
    "Expected Return E(r_S) (%)",
    value=10.0,
    min_value=0.0,
    max_value=50.0,
    step=0.1,
...
st.sidebar.markdown("---")
st.sidebar.info("💡 Adjust the parameters to see how they affect the optimal portfolio.")
...
try:
    ...
    denominator = (
        (E_rB - r_f) * sigma_S**2
        + (E_rS - r_f) * sigma_B**2
        - (E_rB - r_f + E_rS - r_f) * sigma_B * sigma_S * rho_BS
    )
    ...
    sigma_O_squared = (
        w_B**2 * sigma_B**2
        + w_S**2 * sigma_S**2
        + 2 * w_B * w_S * sigma_B * sigma_S * rho_BS
    )
    ...
    with col1:
        ...
        ax.text(-0.5, r_f * 100, r"$r_f$", fontsize=11, ha="right", va="center")
    ...
    with col2:
        ...
        position_type = (
            "Leveraged (Borrowing)"
            if y_star > 1
            else "Conservative (Lending)"
            if y_star < 1
            else "100% in Risky Portfolio"
        )
        ...
        if y_star > 1:
            st.warning(
                f"⚠️ This investor is leveraged, borrowing {abs(alloc_rf)*100:.2f}% to invest more in the risky portfolio."
            )
        elif y_star < 1:
...
st.markdown(
    """
Built for FIN 684 - Investment Analysis and Portfolio Management  
